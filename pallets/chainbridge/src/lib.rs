...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
        ...
        type Proposal: Parameter + Dispatchable<Origin = Self::Origin>  + From<frame_system::Call<Self>> + GetDispatchInfo;
    ...
    #[pallet::type_value]
    pub(super) fn MyDefault() -> u32 { DEFAULT_RELAYER_THRESHOLD}

    #[pallet::storage]
	#[pallet::getter(fn relayer_threshold)]
	pub(super) type RelayerThreshold<T> = StorageValue<_, u32, ValueQuery,MyDefault>;

    #[pallet::storage]
    #[pallet::getter(fn resources)]
    pub(super) type Resources<T> = StorageMap<_, Blake2_128Concat, ResourceId, Option<Vec<u8>>, ValueQuery>;
    ...
    pub(super) type ChainNonces<T> = StorageMap<_, Blake2_128Concat, ChainId, Option<DepositNonce>, ValueQuery>;

	#[pallet::storage]
	pub(super) type Votes<T:Config> = StorageDoubleMap<_, Blake2_128Concat, ChainId, Blake2_128Concat, (DepositNonce,T::Proposal), Option<ProposalVotes<T::AccountId, T::BlockNumber>>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
        ...
        //#[pallet::weight(195_000_000)]
		pub fn eval_vote_state(origin:OriginFor<T>, nonce: DepositNonce, src_id: ChainId, prop: Box<<T as Config>::Proposal>) -> DispatchResult {
